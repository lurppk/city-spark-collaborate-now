          ...
          <div className="hidden md:flex items-center space-x-6">
            {navItems.map((item) => (
              <Button
                key={item.label}
                variant="ghost"
                className="flex items-center space-x-2 text-slate-600 hover:text-blue-600 hover:bg-blue-50 transition-colors"
              >
            ...
            <div className="flex flex-col space-y-3">
              {navItems.map((item) => (
                <Button
                  key={item.label}
                  variant="ghost"
                  className="flex items-center justify-start space-x-3 text-slate-600 hover:text-blue-600 hover:bg-blue-50 w-full"
                >
                  <item.icon className="w-4 h-4" />
                  <span>{item.label}</span>
                  {item.count && (
